...
final class Database 
{
    ...
    public static function last_insert_id()
    {
        ...
        $db = self::$instance->db();
        $result  = $db->getOne( 'SELECT LAST_INSERT_ID();' );
        if ( PEAR::isError( $result )){
    ...
    public static function update( $table, $assoc, $where )
    {
        ...
        $result 
            = $db->autoExecute( $table, $assoc, DB_AUTOQUERY_UPDATE, $where );
    ...
    public static function get_rows( $table, $where=null, Array $vals=array() )
    {
        ...
        $rows   = $peardb->getAll( $sql, $vals, DB_FETCHMODE_ASSOC );
     ...
     * may (and usually should) be provided. It may contain placeholders in 
     * which case an array of values should also be provided. The array of 
     * values must have one value per placeholder.
     *
     * @param mixed $table 
     * @param mixed $where 
     * @param Array $vals 
     ...
     * @return void
     */
    public static function delete ( $table, $where=null, Array $vals=array() )
    {
        $finfo = "delete_rows({$table}, {$where}, {$vals})"; 
                                       ...
                                       Array $vals=array() )
    {
        ...
        $peardb = self::$instance->db();
        $rows   = $peardb->getCol( $sql, $col, $vals );
    ...
    public static function id_exists( $table, $id )
    {
        ...
        $peardb = self::$instance->db();
        $result = $peardb->getOne( $sql, array( $id ) );
